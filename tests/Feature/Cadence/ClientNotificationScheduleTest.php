...
describe('creating a schedule', function (): void {
    it('creates a schedule for a client and points it at the first occurrence', function (ClientNotificationFrequency $frequency): void {
        ...
        assertDatabaseHas('audits', [
            'action' => AuditAction::ClientNotificationScheduleCreated->value,
            'auditable_type' => 'client_notification_schedule',
        ]);
    })->with([
        'one time' => ClientNotificationFrequency::OneTime,
        'monthly' => ClientNotificationFrequency::Monthly,
        'yearly' => ClientNotificationFrequency::Yearly,
    ]);
    ...
    it('refuses a user without the create permission', function (): void {
        ...
        actingAs(administratorWithout([PermissionName::ClientNotificationsCreate]))
            ->post(route('clients.schedules.store', $client), scheduleForm())
            ->assertForbidden();

        expect(ClientNotificationSchedule::query()->count())->toBe(0);
    });
});
...
describe('updating a schedule', function (): void {
    ...
    it('keeps the queued occurrence when only the template changes', function (): void {
        ...
        expect($schedule->template)->toBe(ClientEmailTemplate::GeneralReminder)
    ...
    it('accepts an anchor in the past for an existing schedule', function (): void {
        $schedule = ClientNotificationSchedule::factory()->for(Client::factory())->monthly()->create();
    ...
    it('records the change', function (): void {
        $schedule = ClientNotificationSchedule::factory()->for(Client::factory())->create();

        actingAs(administrator())
            ->put(route('cadence.schedules.update', $schedule), scheduleForm());

        assertDatabaseHas('audits', ['action' => AuditAction::ClientNotificationScheduleUpdated->value]);
    });

    it('refuses a user without the update permission', function (): void {
        $schedule = ClientNotificationSchedule::factory()->for(Client::factory())->create();

        actingAs(administratorWithout([PermissionName::ClientNotificationsUpdate]))
            ->put(route('cadence.schedules.update', $schedule), scheduleForm())
            ->assertForbidden();
    });
});
...
describe('enabling and disabling a schedule', function (): void {
    it('disables a schedule and clears its upcoming occurrence', function (): void {
        ...
        assertDatabaseHas('audits', ['action' => AuditAction::ClientNotificationScheduleDisabled->value]);
    });
    ...
    it('enables a schedule and resumes from the next future occurrence', function (): void {
        ...
        assertDatabaseHas('audits', ['action' => AuditAction::ClientNotificationScheduleEnabled->value]);
    });
...
describe('deleting a schedule', function (): void {
    it('removes the schedule but keeps its delivery history', function (): void {
        ...
        assertDatabaseHas('audits', ['action' => AuditAction::ClientNotificationScheduleDeleted->value]);
    });
    ...
    it('refuses a user without the delete permission', function (): void {
        ...
        actingAs(administratorWithout([PermissionName::ClientNotificationsDelete]))
            ->delete(route('cadence.schedules.destroy', $schedule))
            ->assertForbidden();
