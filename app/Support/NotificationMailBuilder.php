...
final class NotificationMailBuilder
{
     ...
     *
     * A blank the template leaves is filled in first and on its own, because failing to
     * fill one is a different kind of failure from a view that will not render — and it
     * has to be recorded rather than thrown, so the delivery still says what happened.
     */
    public function prepare(
        ...
        try {
            $slotValues = $this->slotValues($dispatch);
        } catch (UnresolvedTemplateSlot $exception) {
        ...
        $data = $this->build($dispatch, $recipient, $occurrence, $slotValues);
     ...
     */
    private function slotValues(NotificationDispatch $dispatch): array
    {
